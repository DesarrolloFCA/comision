...
SELECT b.id_comision,
    b.legajo, b.fecha,
   -- generate_series(b.fecha::timestamp with time zone, b.fecha_fin::timestamp with time zone, '1 day'::interval) AS fecha,
    b.horario,
    b.horario_fin
   FROM (
    SELECT 
        legajo,
        generate_series(fecha::timestamp with time zone, fecha_fin::timestamp with time zone, '1 day'::interval) AS fecha,
        MIN(horario) AS horario,
        MAX(horario_fin) AS horario_fin,
        MIN(id_comision) AS id_comision  -- o usa ARRAY_AGG si quieres mantener todos
    FROM reloj.comision
    GROUP BY legajo, fecha, fecha_fin
) b
     LEFT JOIN reloj.parte a ON a.legajo = b.legajo AND a.fecha_inicio_licencia = b.fecha;
